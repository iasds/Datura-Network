...
- [6. Pre-commit / pre-push hooks (cargo-husky)](#6-pre-commit--pre-push-hooks-cargo-husky)
...
```sh
cd dev-tools && cargo test
```

This only needs to be run **once per fresh clone**, not once per PoC. It
builds the internal `dev-tools` crate, whose only job is pulling in
`cargo-husky` as a dev-dependency; running its test suite is what triggers
`cargo-husky` to install `.git/hooks/pre-commit` and `.git/hooks/pre-push`.
See [`dev-tools/README.md`](dev-tools/README.md) for details.
...
(see below) runs it automatically, scoped only to the crate(s) your commit
actually touches.

## 6. Pre-commit / pre-push hooks (cargo-husky)

The one-time setup in section 3 installs two git hooks via
[`cargo-husky`](https://github.com/rhysd/cargo-husky):

- **pre-commit** - determines which `Poc/<name>/` crate(s) your staged files
  belong to, then runs `cargo fmt --check` and
  `cargo clippy --all-targets -- -D warnings` on just those crates.
- **pre-push** - determines which crate(s) the commits being pushed touched,
  then runs `cargo test` on just those crates.

Both are scoped to touched crates only, so touching one PoC never triggers a
full-repo build across all PoCs.
...
Every PR should include `PR_REPORT.md` at the repo root. Copy it from
`PR_REPORT_TEMPLATE.md` (or just push once - the pre-push hook will create
it for you automatically if it doesn't exist yet on your branch) and fill
in the "What changed and why," "Manual testing notes," "AI / LLM usage
disclosure," and "Known limitations" sections by hand. The "Automated
check results" section at the bottom is written by the pre-push hook
itself, using real `cargo fmt`/`clippy`/`test` output for the crates your
branch touches - it refreshes on every push, so re-push after your last
code change before opening the PR to make sure the report reflects your
final state. Don't hand-edit anything below the
`<!-- AUTO-GENERATED:START -->` marker; it gets overwritten.

`PR_REPORT.md` is per-branch, disposable scaffolding for review, not
permanent project documentation - whoever merges the PR should delete it
as part of the merge (or as a quick follow-up commit) so it doesn't pile
up on `main`.
...
- `PR_REPORT.md` is present, its automated section is current (matches
  your latest push), and you've filled in the manual sections.
