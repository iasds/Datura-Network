...
fn main() -> io::Result<()> {
    ...
    println!("Generated {} additional Hashring nodes, each with a {} character-long identifier.",count,length);
    ...
    println!("The Hidden service gdatura24gtdy23lxd7ht3xzx6mi7mdlkabpvuefhrjn4t5jduviw5ad.dn's hash is {}",hiddenservice_hash);
    let (pos, nearest) = locate_nearest(hiddenservice_hash, "hashring_sorted.txt")?;

    println!("The Hidden Service's rendezvous node DNS knowledge is most likely at the {}th position on the hashring.", pos);
    println!("here are the 8 closest hashring neighbors of {} (that are the most likely to know where the RDV node of the hidden service is):",hiddenservice_hash);
    for s in nearest {
        println!("  {s}");
    }
    Ok(())
...
pub fn locate_nearest(
    ...
    while taken < 8 && total > neighbours.len() {
        ...
        let wrap_idx = (taken % total) as usize;
