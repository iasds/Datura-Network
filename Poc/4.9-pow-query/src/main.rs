...
fn main() {
	...
	let mut port = 9051;
	if args.len() > 2 { port = args[2].parse().unwrap(); }
	else if args.len() > 1 { port = args[1].parse().unwrap(); }
			...
			Ok(mut stream) => {
				...
				match stream.read_exact(&mut challenge_buf) {
					...
					Ok(_) => {
						...
						println!("got challenge '{:X}' of difficulty {} from server", u128::from_le_bytes(challenge_buf), get_challenge_effort(challenge));
						...
						if args[1].parse::<String>().unwrap() == "random" {
							...
							solution = solve_challenge(thread::available_parallelism().unwrap().get(), challenge);
						...
						match stream.read(&mut node_list_buf) {
							...
							Ok(amt) => {
								println!("server replied with {} bytes \n\"{}\"", amt, from_utf8(&node_list_buf[..amt]).unwrap());
							}
						}
					}
				}
...
127.0.0.1:20001";
		...
		for stream in tcp_listener.incoming() {
			...
			std::thread::spawn(move || {
				...
				println!("{} connected, sent difficulty {} challenge '{:X}'", peer_addr, CHALLENGE_DIFFICULTY, challenge);
				...
				match stream.read_exact(&mut solution_buf) {
					...
					Ok(_) => {
						...
						if verify_solution(CHALLENGE_DIFFICULTY, challenge, solution_buf) {
							println!("solution from {} was correct, sending list of nodes", peer_addr);

							stream.write(hardcoded_node_list.as_bytes()).unwrap();
						} else {
