...
impl SecureAllocationSystem {
    ...
    pub fn calculate_allocation(&self, clients: &mut [Consumer]) {
        ...
        for client in clients.iter() {
            ...
            let contribution = client.contribution as f64;
            let compressed_contribution = contribution.powf(self.compression_exponent);
            let reserve = client_reserve_rate * compressed_contribution;
        ...
        for client in clients.iter() {
            let contribution = client.contribution as f64;
            let compressed_contribution = contribution.powf(self.compression_exponent);
            compressed.insert(client.id, compressed_contribution);
            total_compressed += compressed_contribution;
        }
...
mod tests {
    ...
    fn test_allocation_qualified_clients() {
        let mut manager = ResourceManager::new(1000.0);

        let clients = vec![Consumer::new(1, 50_000, 5), Consumer::new(2, 100_000, 10)];
    ...
    fn test_allocation_comprehensive() {
        ...
        assert!(
            total_allocated <= capacity * 1.001, // Small floating point tolerance
            "Total allocation {} exceeds capacity {}",
            total_allocated,
            capacity
        );
        ...
        assert!(
            allocated[0].allocation > 0.0,
            "Client 1 (qualified) should get allocation"
        );
        assert!(
            allocated[1].allocation > 0.0,
            "Client 2 (qualified) should get allocation"
        );
        assert!(
            allocated[2].allocation > 0.0,
            "Client 3 (qualified) should get allocation"
        );
        assert!(
            allocated[4].allocation > 0.0,
            "Client 5 (qualified) should get allocation"
        );

        // Verify unqualified client still gets allocation (from earned share, not reserve)
        assert!(
            allocated[3].allocation > 0.0,
            "Client 4 (unqualified) should get earned allocation"
        );

        // Verify higher contribution gives higher allocation
        assert!(
            allocated[2].allocation > allocated[0].allocation,
            "Higher contribution (200k > 50k) should yield higher allocation"
        );
        assert!(
            allocated[2].allocation > allocated[4].allocation,
        ...
        assert!(
            allocated[1].allocation > allocated[4].allocation,
    ...
    fn test_allocation_reserve_calculation() {
        ...
        assert!(
            allocated[1].allocation > allocated[0].allocation,
        ...
        assert!(
            ...
            "Tenure shouldn't cause >2x difference at same contribution"
        );
    ...
    fn test_allocation_compression_effect() {
        ...
        assert!(
            ratio > 2.5,
            "10x contribution should yield significant allocation increase"
        );
        assert!(
            ratio < 8.0,
            "Compression should prevent linear allocation scaling"
        );
    }
    ...
    fn test_allocation_empty_clients() {
        ...
        let result = manager.allocate(clients);
    ...
    fn test_allocation_all_unqualified() {
        ...
        assert!(
            total > 0.0,
            "Unqualified clients should still share capacity"
        );
    ...
    fn test_allocation_fairness_proportional() {
        ...
        assert!(
            ratio_1_2 > 1.8 && ratio_1_2 < 2.2,
            "Allocation ratio 1:2 should hold (got {})",
            ratio_1_2
        );
        assert!(
            ratio_1_3 > 3.6 && ratio_1_3 < 4.4,
            "Allocation ratio 1:4 should hold (got {})",
            ratio_1_3
        );
    }
}
