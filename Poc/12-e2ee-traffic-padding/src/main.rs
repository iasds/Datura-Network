...
const NUM_PACKETS_KEM_CIPHERTEXT: usize =
    (CIPHERTEXT_LEN as f32 / PACKET_SIZE as f32).ceil() as usize;
...
fn send_equal_data_per_packet(stream: &mut TcpStream, data: &[u8]) -> usize {
    /*
    ...
    for _ in 0..num_packets {
        stream
            .write_all(&data[start..(start + PACKET_SIZE)])
            .unwrap();
        start += PACKET_SIZE;
    }
    start
...
fn receive_packets(stream: &mut TcpStream, out_buf: &mut Vec<u8>) -> usize {
    ...
    loop {
        ...
        let bytes_read = stream.read(&mut packet_buf).unwrap();

        if bytes_read == 0 {
            return 0;
        }

        // Extend after checking bytes_read == 0, so if 0 bytes are read, then that doesn't
        // get added to vector
        out_buf.extend(&packet_buf);
    ...
    if role == "server" {
        ...
        for stream in listener.incoming() {
            ...
            let (server_private_key, server_public_key) =
                hpke.generate_key_pair().unwrap().into_keys();
            let mut padded_public_key: Vec<u8> = server_public_key.as_slice().to_vec();
            ...
            padded_public_key.extend_from_slice(&[0_u8; KEM_PUBKEY_PADDING_AMOUNT]);
            send_equal_data_per_packet(&mut stream, &padded_public_key);
            println!(
                "[Node B] Sent encapsulation key to Node A in {:?}",
                now.elapsed()
            );

            // Receive padded ciphertext.

            let mut padded_ciphertext: Vec<u8> = Vec::new();
            for _ in 0..NUM_PACKETS_KEM_CIPHERTEXT {
                let mut packet_buf: [u8; PACKET_SIZE] = [0; PACKET_SIZE];
                stream.read_exact(&mut packet_buf).unwrap();
                padded_ciphertext.extend(packet_buf);
            }

            println!("[Node B] Received ciphertext from Node A");

            // X-Wing ciphertext len is 1120 bytes, so chop of extra padding
            let ciphertext = &padded_ciphertext[..CIPHERTEXT_LEN];

            let now = std::time::Instant::now();
            let mut receiver_context = hpke
                .setup_receiver(ciphertext, &server_private_key, b"", None, None, None)
                .unwrap();
            ...
            let len_padding: i32 =
                (top_half_len_padding << 8 & 0xff00) | (bottom_half_len_padding & 0xff);
        ...
        for stream in listener.incoming() {
            ...
            let mut send_stream =
                TcpStream::connect(format!("127.0.0.1:{}", server_listen_port)).unwrap();

            // Receive KEM pubkey and setup ciphertext
            let mut padded_public_key: Vec<u8> = Vec::new();
            for _ in 0..NUM_PACKETS_KEM_PUBKEY {
                let mut packet_buf: [u8; PACKET_SIZE] = [0; PACKET_SIZE];
                send_stream.read_exact(&mut packet_buf).unwrap();
                padded_public_key.extend(packet_buf);
            }
            ...
            let server_public_key =
                HpkePublicKey::new(padded_public_key[..KEM_PUBKEY_LEN].to_vec());
            let (ciphertext, mut sender_context) = hpke
                .setup_sender(&server_public_key, b"", None, None, None)
                .unwrap();

            let mut padded_ciphertext: Vec<u8> = Vec::new();
            padded_ciphertext.extend_from_slice(&ciphertext);
            padded_ciphertext.extend_from_slice(&[0_u8; KEM_C_PADDING_AMOUNT]);

            // Send ciphertext to server
            send_equal_data_per_packet(&mut send_stream, &padded_ciphertext);
            println!(
                "[Node A] Created and sent KEM ciphertext to Node B in {:?}",
                now.elapsed()
            );
            ...
            for bytes_from_end in 1..=AEAD_TAG_SIZE {
                msg.remove(msg_len - bytes_from_end);
            }
            ...
            let bottom_half_len_padding = (padding_len & 0xff) as u8;
            ...
            let now = std::time::Instant::now();
            // No AAD
            let msg_encrypted: Vec<u8> = sender_context.seal(b"", &msg).unwrap();
