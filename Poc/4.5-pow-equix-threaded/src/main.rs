...
fn solve_challenge(num_threads: usize, challenge: u128) -> [u8; 24] {
    ...
    for thread_index in 0..num_threads {
        ...
        let mut salt: u64 = search_pos + search_inc * (thread_index as u64);
    ...
    unreachable!(
        "all solver threads returned None, but a thread only returns None after observing \
         `done == true`, which is only ever set by a thread that already returned Some(solution)"
    )
}

fn verify_solution(effort: u32, challenge: u128, solution: [u8; 24]) -> bool {
    // extract effort parameter from challenge
    if ((challenge & 0xffffffff) as u32) != effort {
        return false;
    };
...
fn main() {
    ...
    print!(
        "How many threads should i use? (Press Enter for default of {}): ",
        num_threads
    );
    stdout().flush().unwrap();
    ...
    if picked_num_threads < 1 || picked_num_threads > num_threads {
        println!("Thread amount {} is out of bounds", picked_num_threads);
        exit(1);
    }
    println!("\nRunning with {} thread(s)\n", picked_num_threads);

    // benchmark at various effort values
    for effort_level in 0..8 {
        let effort = effort_level * 64;

        let mut solve_sum: Duration = Duration::from_secs(0);
        let mut verify_sum: Duration = Duration::from_secs(0);

        print!(
            "Solving {} challenges of effort {} ",
            BENCHMARK_SAMPLES, effort
        );
        stdout().flush().unwrap();
        for _ in 0..BENCHMARK_SAMPLES {
            ...
            let now = std::time::Instant::now();
            assert!(verify_solution(effort, challenge, solution));
            let took = now.elapsed();
            verify_sum += took;
        ...
        println!(
            ...
            solve_sum / BENCHMARK_SAMPLES,
            verify_sum / BENCHMARK_SAMPLES
        );
    }
