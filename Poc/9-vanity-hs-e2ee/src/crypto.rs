...
pub fn ed25519_to_hpke_public(vk: &VerifyingKey) -> HpkePublicKey {
    HpkePublicKey::new(vk.to_montgomery().to_bytes().to_vec())
}
...
pub fn ed25519_to_hpke_private(sk: &SigningKey) -> HpkePrivateKey {
...
pub fn encrypt(
    ...
    let pk = ed25519_to_hpke_public(recipient_vk);
    let (kem_output, mut ctx) = make_hpke().setup_sender(&pk, INFO, None, None, None)?;
    let ciphertext = ctx.seal(b"", plaintext)?;
...
pub fn decrypt(
    ...
    let privkey = ed25519_to_hpke_private(sk);
    let mut ctx = make_hpke().setup_receiver(kem_output, &privkey, INFO, None, None, None)?;
    ctx.open(b"", ciphertext)
}
...
mod tests {
    ...
    fn encrypt_decrypt_roundtrip() {
        ...
        let (kem_output, ciphertext) = encrypt(&vk, message).expect("encrypt failed");

        assert_eq!(kem_output.len(), KEM_OUTPUT_LEN);
        assert_eq!(ciphertext.len(), message.len() + 16); // "ChaCha20Poly1305" tag

        let decrypted = decrypt(&sk, &kem_output, &ciphertext).expect("decrypt failed");
    ...
    fn wrong_key_fails() {
        ...
        let (kem_output, ciphertext) = encrypt(&vk, b"secret").expect("encrypt failed");
        assert!(decrypt(&wrong_sk, &kem_output, &ciphertext).is_err());
    }
    ...
    fn tampered_ciphertext_fails() {
        ...
        let (kem_output, mut ciphertext) = encrypt(&vk, b"secret").expect("encrypt failed");
        ciphertext[0] ^= 0xff;
        assert!(decrypt(&sk, &kem_output, &ciphertext).is_err());
    }
    ...
    fn empty_ciphertext_fails() {
        ...
        let sk = SigningKey::generate(&mut OsRng);
        let vk = sk.verifying_key();
        let (kem_output, _) = encrypt(&vk, b"x").expect("encrypt failed");
        assert!(decrypt(&sk, &kem_output, &[]).is_err());
    }
    ...
    fn short_ciphertext_fails() {
        ...
        let vk = sk.verifying_key();
        let (kem_output, _) = encrypt(&vk, b"x").expect("encrypt failed");
        assert!(decrypt(&sk, &kem_output, &[0u8; 15]).is_err());
    }
    ...
    fn invalid_kem_output_length_fails() {
        ...
        let sk = SigningKey::generate(&mut OsRng);
        let vk = sk.verifying_key();
        let (_, ciphertext) = encrypt(&vk, b"x").expect("encrypt failed");
        // Too short
        assert!(decrypt(&sk, &[0u8; 16], &ciphertext).is_err());
        // Too long
        assert!(decrypt(&sk, &[0u8; 64], &ciphertext).is_err());
    }
    ...
    fn encrypt_empty_plaintext() {
        ...
        let sk = SigningKey::generate(&mut OsRng);
        let vk = sk.verifying_key();
        let (kem_output, ciphertext) = encrypt(&vk, b"").expect("encrypt failed");
